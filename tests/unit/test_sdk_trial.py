"""Contract tests for sdk.trial.run_trial and its TrialRun wrapper."""
...
GENERATION_ID = "OTPGEN-sdk-trial-test"
POPULATION_SIZE = 6
...
def build_population(tmp_path: Path) -> Path:
    ...
    store.write_population(
        GENERATION_ID,
        population_id="sdk-trial-demo",
        source_request=document(
            "opentrials.osp.population-worker-request", {"population_id": "sdk-trial-demo"}
        ),
...
def two_arm_trial() -> Trial:
    return Trial(
        trial_id="SDK-DOSE-COMPARISON",
        title="SDK dose comparison",
        question_of_interest="How do outcomes differ across arms?",
        population=PopulationSpec(
            id="sdk-trial-demo", size=POPULATION_SIZE, seed=1, generator_version="0.1.0"
        ),
...
def test_run_trial_returns_a_working_trial_run(
    ...
    assert {record.arm_id for record in run.endpoints} == {"low", "high"}

    summary = run.summary()
    assert "OpenTrials trial run" in summary

...
**v0.7-A — Generic model capability + registration** adds a deliberately engine-agnostic `ModelCapabilityProfile` (`models/capability.py`) declaring what one registered model actually supports — compounds, administration routes and their mutable parameters, verified physiology targets, canonical output mappings, and explicit reasoned unsupported-capability entries — as data a generic pipeline can query, plus a minimal `ModelCapabilityRegistry`. Aciclovir is registered as the first profile, with every value cross-checked against the still-unchanged orchestration constants by a dedicated drift-guard test. Orchestration itself has not been rewired to consume the profile yet; that, and a second real model proving the abstraction generalizes, are the next two chunks of v0.7.
